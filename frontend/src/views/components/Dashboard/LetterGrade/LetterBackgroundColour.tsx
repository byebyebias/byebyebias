...
function LetterBackgroundColour({
	score,
	percentage,
}: LetterBackgroundColourProps) {
  ...
  */
	const getBackgroundColor = (percentage: number) => {
		...
		const selectedRange = ranges.find(
			(range) =>
				percentage >= range.rangeStart && percentage <= range.rangeEnd,
		);
		...
		const red = Math.round(
			start.r + relativePercentage * (end.r - start.r),
		);
		const green = Math.round(
			start.g + relativePercentage * (end.g - start.g),
		);
		const blue = Math.round(
			start.b + relativePercentage * (end.b - start.b),
		);
