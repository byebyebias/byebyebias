...
export class S3LinkUploadController {
	...
	constructor(
		interactor: S3LinkUploadInteractor,
		presenter: S3LinkUploadPresenter,
	) {
		this.interactor = interactor;
		this.presenter = presenter;
	}

	async handleS3Link(s3Link: string, protected_attributes: Array<string>) {
		try {
			const result = await this.interactor.processLink(
				s3Link,
